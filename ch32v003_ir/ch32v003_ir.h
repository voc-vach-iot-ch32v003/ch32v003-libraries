 ...
 * @file ch32v003_ir.h
 * @author Vọc Vạch IoT
 * @brief Thư viện thu phát hồng ngoại đa giao thức (NEC, Sony, Samsung) cho
 * CH32V003.
 * @version 2.0
 * @date 2026-06-07
 */
...
#define IR_RAW_BUFFER_SIZE                                                              \
  256 /**< Số lượng sườn xung (Edges) tối đa có thể bắt được trong một khung \
         truyền. */
#endif

#ifndef IR_MAX_SUPPORTED_BITS
#define IR_MAX_SUPPORTED_BITS                                                   \
  80 /**< Số lượng BIT dữ liệu tối đa mà thư viện có thể giải mã và quản lý. \
      */
#endif

#define IR_RAW_DATA_BYTES                                                                \
  ((IR_MAX_SUPPORTED_BITS + 7) /                                                         \
   8) /**< Tự động tính toán số BYTE cần thiết trong struct IR_Data_t (Chia \
         tròn lên). */
 ...
 * @note Dùng để map giữa dữ liệu động trong RAM và bảng tra cứu tĩnh trong
 * Flash.
 */
typedef enum {
  IR_UNKNOWN = 0,
...
/**
 * @brief Kiểu mã hóa bit của giao thức (Bắt buộc phải phân loại vì Sony khác
 * NEC)
 */
typedef enum {
  IR_ENCODE_DISTANCE, /**< Mã hóa bằng độ rộng khoảng lặng - Space Distance
                         (NEC, Samsung, TCL) */
 ...
 */
typedef enum {
  IR_ORDER_LSB_FIRST = 0, // Bit thấp (Bit 0) được gửi/nhận trước (Mặc định:
                          // NEC, Samsung, TCL...)
  IR_ORDER_MSB_FIRST =
      1 // Bit cao (Bit 7) được gửi/nhận trước (Một số dòng tinh chỉnh đặc biệt)
} IR_BitOrder_t;
...
typedef struct {
  ...
  uint8_t totalBits; /**< Tổng số lượng bit của giao thức (Hỗ trợ tối đa 64 bit,
                        ví dụ TLC = 48) */
...
/**
 * @brief Cấu trúc dữ liệu chứa mảng xung thô (Raw Buffer) cho tính năng học
 * lệnh.
 */
typedef struct {
  uint16_t rawBuf[IR_RAW_BUFFER_SIZE]; /**< Mảng lưu độ rộng thời gian của các
                                          khoảng tick xung (us) */
...
/**
 * @brief Quét và lưu trữ dữ liệu xung thô từ tín hiệu hồng ngoại vào bộ đệm
 * (Hàm chặn).
 * @param mcu_pin Chân vật lý MCU làm đầu vào thu hồng ngoại, chọn từ @ref
 * MCUPin_t hoặc @ref MCUSpecialPin_t.
 * @param rawData Con trỏ tới cấu trúc @ref IR_RawData_t để lưu kết quả độ rộng
 * xung thô.
 * @return 1 nếu đã thu thập được dữ liệu thô thành công, 0 nếu không có tín
 * hiệu hoặc bộ đệm đầy.
 */
uint8_t irReadRaw(uint8_t mcu_pin, IR_RawData_t *rawData);

/**
 * @brief Đọc và giải mã dữ liệu hồng ngoại trực tiếp từ chân vật lý chỉ định
 * (Hàm chặn).
 * @param mcu_pin Chân vật lý MCU làm đầu vào thu hồng ngoại, chọn từ @ref
 * MCUPin_t hoặc @ref MCUSpecialPin_t.
 * @param irData Con trỏ tới cấu trúc @ref IR_Data_t để lưu thông tin sau giải
 * mã.
 * @return 1 nếu nhận dạng và giải mã thành công, 0 nếu không có tín hiệu hoặc
 * nhiễu.
 */
uint8_t irRead(uint8_t mcu_pin, IR_Data_t *irData);

/**
 * @brief Phát tín hiệu hồng ngoại theo cấu trúc xung thô đã thu thập (Học lệnh)
 * ra chân vật lý (Hàm chặn).
 * @param mcu_pin Chân vật lý MCU kết nối mạch đệm LED phát IR, chọn từ @ref
 * MCUPin_t hoặc @ref MCUSpecialPin_t.
 * @param rawData Con trỏ tới cấu trúc @ref IR_RawData_t chứa dữ liệu học lệnh
 * thô.
 */
void irSendRaw(uint8_t mcu_pin, const IR_RawData_t *rawData);

/**
 * @brief Phát tín hiệu điều khiển hồng ngoại theo giao thức chuẩn hóa ra chân
 * vật lý MCU.
 * @param mcu_pin Chân vật lý MCU kết nối mạch đệm LED phát IR, chọn từ @ref
 * MCUPin_t hoặc @ref MCUSpecialPin_t.
 ...
 */
void irSend(uint8_t mcu_pin, IR_Protocol_t protocol, uint16_t address,
            uint32_t command);
...
/**
 * @brief Quét và giải mã tín hiệu hồng ngoại dựa theo cấu trúc Port và Số pin
 * chỉ định (Hàm chặn).
 ...
 */
uint8_t irReadPort(const GPIO_TypeDef *GPIOx, uint8_t pinNumber,
                   IR_Data_t *irData);

/**
 * @brief Quét và lưu trữ dữ liệu xung thô từ tín hiệu hồng ngoại vào bộ đệm,
 * không giải mã (Hàm chặn).
 * @param GPIOx Con trỏ quản lý vùng nhớ Port (GPIOA, GPIOC, GPIOD).
 * @param pinNumber Số thứ tự bit của chân trong Port (Giá trị từ 0 đến 7).
 * @param rawData Con trỏ tới cấu trúc lưu mảng độ rộng xung và số lượng xung đã
 * lưu.
 * @return 1 nếu đã thu thập được dữ liệu thô thành công, 0 nếu không có tín
 * hiệu hoặc bộ đệm đầy.
 */
uint8_t irReadPortRaw(const GPIO_TypeDef *GPIOx, uint8_t pinNumber,
                      IR_RawData_t *rawData);

/**
 * @brief Phát sóng mang hồng ngoại theo đúng cấu trúc giao thức ra chân Port
 * chỉ định.
 * @param GPIOx Con trỏ quản lý vùng nhớ Port (GPIOA, GPIOC, GPIOD).
 * @param pinNumber Số thứ tự bit của chân trong Port (Giá trị từ 0 đến 7).
 ...
 */
void irSendPort(GPIO_TypeDef *GPIOx, uint8_t pinNumber, IR_Protocol_t protocol,
                uint16_t address, uint32_t command);

/**
 * @brief Phát tín hiệu hồng ngoại theo cấu trúc xung thô đã thu thập được,
 * không cần giải mã (Hàm chặn).
 * @param GPIOx Con trỏ quản lý vùng nhớ Port (GPIOA, GPIOC, GPIOD).
 * @param pinNumber Số thứ tự bit của chân trong Port (Giá trị từ 0 đến 7).
 * @param rawData Con trỏ tới cấu trúc chứa dữ liệu xung thô đã thu thập được từ
 * hàm irReadPortRaw.
 */
void irSendPortRaw(GPIO_TypeDef *GPIOx, uint8_t pinNumber,
                   const IR_RawData_t *rawData);
